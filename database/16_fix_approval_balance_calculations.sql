-- ==============================================================================
-- Migration: Fix balance calculation bugs in approve_leave_request
-- Date: 2026-03-25
-- Fixes:
--   1. days_count recalculation was ignoring segments and half-days
--   2. RH date editing didn't reset balance_conge_used/balance_recuperation_used
--   3. count_working_days was called with hardcoded 'FULL' instead of actual half-day values
-- ==============================================================================

-- Drop and recreate approve_leave_request with fixes
...
BEGIN
  ...
  -- FIX #2: RH can edit dates at PENDING stage — now properly recalculates and resets split
  IF v_field = 'rp' AND p_new_start_date IS NOT NULL AND p_new_end_date IS NOT NULL THEN
    ...
    BEGIN
      ...
      UPDATE leave_requests SET
        ...
        -- Reset split when dates change: for non-mixed, reassign to the request_type;
        -- for mixed, clear both (segments are now invalid after date change)
        balance_conge_used = CASE
          WHEN COALESCE(v_request.is_mixed, false) THEN NULL
          WHEN v_request.request_type = 'CONGE' THEN v_new_days
          ELSE 0
        END,
        balance_recuperation_used = CASE
          WHEN COALESCE(v_request.is_mixed, false) THEN NULL
          WHEN v_request.request_type = 'RECUPERATION' THEN v_new_days
          ELSE 0
        END
  ...
  IF v_next_status = 'APPROVED' THEN
    ...
    -- FIX #1 & #3: Use stored split if available, otherwise recalculate with actual half-days
    ...
    IF v_conge_used > 0 OR v_recup_used > 0 THEN
      -- Trust the stored split (from segments or frontend submission)
      v_days := v_conge_used + v_recup_used;
      UPDATE leave_requests SET days_count = v_days WHERE id = p_request_id;
    ELSE
      -- Legacy fallback: no split stored, recalculate with actual half-day values
