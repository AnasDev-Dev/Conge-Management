...
- [x] **Dérogation support** — allow requests when congé shortage ≤ 3 days, balance goes negative (max -3), monthly accrual absorbs. Clear info: "Xj demandés, Yj indisponibles, solde: -Yj"
- [x] **Date overlap detection** — blocks requests that overlap with existing approved/pending leave, shown as inline red error on segment card
- [x] **0 working days warning** — amber warning when selected dates fall entirely on weekends/holidays
- [x] **Recovery expiration blocking** — cannot request RECUPERATION if end date exceeds lot expiration, with expiration alert for soon-expiring lots
...
- [x] **Table/card view toggle** — switch between table rows (default) and card grid across all 3 tabs (congé, exceptionnel, maladie)
...
- [x] **Nights calculation (nuitées)** — duration counted as nights not days (1st–5th = 4n, +0.5 travel)
- [x] **PEC/non-PEC date segments** — INTERNATIONAL missions support multiple date segments, each with independent PEC toggle, hotel amount, meal counts, and subtotal
- [x] **PEC segments stored in DB** — `pec_segments` JSONB column preserves full breakdown
- [x] **PEC segments on detail page** — per-segment display with dates, nights, meals, subtotals, and grand total
- [x] **2-page print layout** — Page 1: mission details (no prices), Page 2: total amount + director signature only
- [x] Pipeline progress shows "Xn" (nights) on list cards

### Mission Validation Board (`/dashboard/mission-validations`)
- [x] **2-stage pipeline**: Responsable Administratif > Directeur Exécutif (changed April 2026)
- [x] Auto-skip Resp.Admin step if creator is RESPONSABLE_ADMIN
- [x] Approve / Reject with undo support
- [x] **RESPONSABLE_ADMIN role** — new role for mission-specific validation (department-scoped)

### Recovery Requests (`/dashboard/recovery-requests`)
- [x] Employee submission form: days (0.5–5), date worked, work type, reason
- [x] Work types: Jour Férié, Jour de Repos, Samedi, Dimanche, Mission, Autre
...
- [x] **Table row layout** — requests displayed as table rows (not cards) with columns for employee, period, days, type, status
- [x] **Mission auto-link** — approved missions shown in dropdown, auto-fills dates/reason/type when linked
...
- [x] **Holidays tab** — Recurring vs variable (grouped by year), duplicate detection, calendar date picker, delete fix. **Auto-recalculates** active leave requests spanning added/edited/deleted holidays via `recalculate_leave_for_holiday()` RPC
...
- [x] **Recovery lots breakdown** — per-lot display with remaining days, year acquired, expiration date, and "Expire bientot" badge for soon-expiring lots
...
| `mission_requests` | Mission orders with approval chain, `pec_segments` JSONB for PEC breakdown |
...
### Mission Approval Chain (2-stage, April 2026)
```
PENDING → VALIDATED_DC (Responsable Administratif) → APPROVED (Directeur Exécutif)
         Auto-skip Resp.Admin if creator is RESPONSABLE_ADMIN
         ADMIN bypasses all stages
```
...
| `migrations/20260409_00_clean_all_data.sql` | Nuclear cleanup — truncate all data for production reset |
| `migrations/20260409_seed_real_employees.sql` | 29 real employees + 4 external N+1s + admin (from client CSV) |
| `migrations/20260409_departments_and_assignments.sql` | 3 ATH + 5 FRMG departments with employee assignments |
| `migrations/20260409_revert_to_role_based_pipeline.sql` | Restore original role-based leave approval pipeline |
| `migrations/20260410_recalculate_leave_on_holiday.sql` | `recalculate_leave_for_holiday()` RPC for REQ-6 |
| `migrations/20260411_mission_2stage_responsable_admin.sql` | RESPONSABLE_ADMIN role + 2-stage mission pipeline + updated `is_manager()` / `can_manage_user()` |
| `migrations/20260413_mission_pec_segments_column.sql` | `pec_segments` JSONB column on `mission_requests` |
...
| `RH` | All requests, leave Kanban stage 1, employee directory, settings, balance init, recovery validation, notifications |
| `CHEF_SERVICE` | Department requests, leave Kanban stage 2, employee directory, recovery validation (dept), notifications |
| `RESPONSABLE_ADMIN` | Department requests, **mission Kanban stage 1**, leave Kanban stage 2, employee directory, recovery validation (dept), notifications. Same leave permissions as CHEF_SERVICE. Cross-company via `user_company_roles` |
| `DIRECTEUR_EXECUTIF` | All requests, leave Kanban stage 3 + **mission Kanban stage 2** (final approval), employee directory, notifications |
| `ADMIN` | Full access, all Kanban stages, settings, employee creation. **ADMIN role hidden from employee create/edit UI** (only assignable via DB) |
...
├── print-mission-document.tsx        Printable mission order (2-page A4): details + total/director signature
...
## Recently Completed (April 2026 — Client Requirements V2)

| # | Requirement | Status | Notes |
|---|-------------|--------|-------|
| REQ-1 | Mission → recovery link | ✅ Done | Auto-link approved missions in recovery form (dates, reason, type) |
| REQ-2 | Recovery as table rows | ✅ Done | Table layout with columns: employee, period, days, type, status |
| REQ-3 | Recovery expiration in profile | ✅ Done | Per-lot display with remaining days, year, expiration date, expiring-soon badge |
| REQ-4 | Block expired recovery + alert | ✅ Done | Block RECUP if end_date > lot expiration, amber alert for expiring lots |
| REQ-5 | Negative balance derogation (-3) | ✅ Done | Allow requests when shortage ≤ 3 days, balance goes to -3 max, monthly accrual absorbs |
| REQ-6 | Holiday recalculates requests | ✅ Done | `recalculate_leave_for_holiday()` RPC called on holiday add/edit/delete, toast shows affected count |
| REQ-7 | Remove ADMIN from role picker | ✅ Done | ADMIN removed from create/edit employee dialogs |
| REQ-8 | Mission nights (nuitées) | ✅ Done | Duration = nights not days (1st→5th = 4n), +0.5 travel bonus |
| REQ-9 | Mission PEC segments | ✅ Done | Multiple date segments per mission (PEC/non-PEC), stored in `pec_segments` JSONB, displayed on detail |
| REQ-10 | Mission 2-stage pipeline | ✅ Done | New `RESPONSABLE_ADMIN` role, pipeline: Resp.Admin → Director (removed RH+Chef from missions) |
| REQ-11 | Mission print 2 pages | ✅ Done | Page 1: details (no prices), Page 2: total + director signature only |

### Other April 2026 Changes
| Feature | Status | Notes |
|---------|--------|-------|
| Table/card view toggle on demands page | ✅ Done | Switch between table (default) and cards across congé/exceptionnel/maladie tabs |
| Date overlap detection | ✅ Done | Blocks leave requests overlapping with existing approved/pending requests, inline red error |
| 0 working days warning | ✅ Done | Amber inline warning when dates fall entirely on weekends/holidays |
| Employee edit dept sync | ✅ Done | Home assignment's department synced with main form on save |
| Real employee data seeded | ✅ Done | 29 real employees + 4 external N+1s + admin, with balances from client CSV |
| Department structure | ✅ Done | 3 ATH + 5 FRMG departments with employee assignments |
| Segment alternation UX | ✅ Done | New segment always alternates type (RECUP→CONGE, CONGE→RECUP) |

## Previously Completed (March 2026)

| Feature | Status | Notes |
|---------|--------|-------|
| Segment-based mixed requests | ✅ Done | 4-step wizard with segment builder, per-day `leave_request_details`, split balance deduction |
| Recovery expiration automation | ✅ Done | pg_cron daily, notifications on expiry + 30-day warnings, dashboard banner |
| 52-day congé cap enforcement | ✅ Done | `MAX_CONGE_BALANCE = 52` enforced in accrual calc, balance-init UI/API |
| Per-request signatures | ✅ Done | `SignatureDialog` component (draw/upload/saved), stored per approval stage |
| Balance Model V2 | ✅ Done | Carry-over + monthly accrual, department-based entitlement, seniority bonus |
...
| Email notifications on approval/rejection | Medium | Currently in-app only |
| Segment editor for approvers on Kanban | Low | Display done (badges), but approvers can't yet edit individual segments during validation |
